...
#define TXD1_PORT_SETTING() do{ \
        PORT->P13CFG = 0x0A;        /* allocate TXD1 to P00 */ \
        PORT->P1   |=  (1 << 3);    /* P00 output high level */ \
        PORT->PM1  &= ~(1 << 3);    /* P00 is used as TXD1 output */ \
        PORT->POM1 &= ~(1 << 3);    /* P00 is normal output mode */ \
        PORT->PMC1 &= ~(1 << 3);    /* P00 digital function */ \
}while(0)

/* ToDo: You can allocate the RXD1 to any desired pins with RXD1PCFG register */
#define RXD1_PORT_SETTING() do{ \
        PORT->RXD1PCFG = 0x07;      /* allocate RXD1 to P01 */ \
        PORT->PM1  |=  (1 << 4);    /* P01 is used as RXD1 input */ \
        PORT->PMC1 &= ~(1 << 4);    /* P01 digital function */ \
}while(0)
...
#define INTP0_PORT_SETTING() do{ \
        PORT->INTP0PCFG = 0x03;     /* allocate INTP0 to P136 */ \
        PORT->PM1  |=  (1 << 0);   /* P136 is used as INTP0 input */ \
        PORT->PMC1 &= ~(1 << 0);   /* P136 digital function */ \
}while(0)
