...
#define TO11_PORT_SETTING() do{ \
        PORT->P11CFG = 0x02;	    /* allocate TO11 to P15 */ \
        PORT->P1   &= ~(1 << 1);    /* P15 output low level */ \
        PORT->PM1  &= ~(1 << 1);    /* P15 is used as TO11 output */ \
        PORT->POM1 &= ~(1 << 1);    /* P15 is normal output mode */ \
        PORT->PMC1 &= ~(1 << 1);    /* P15 digital function */ \
}while(0)

/* ToDo: You can allocate the TO12 to any desired pins with PxxCFG register */
#define TO12_PORT_SETTING() do{ \
        PORT->P22CFG = 0x03;	    /* allocate TO12 to P50 */ \
        PORT->P2   &= ~(1 << 2);    /* P50 output low level */ \
        PORT->PM2  &= ~(1 << 2);    /* P50 is used as TO12 output */ \
        PORT->POM2 &= ~(1 << 2);    /* P50 is normal output mode */ \
...
#define TO13_PORT_SETTING() do{ \
        PORT->P12CFG = 0x04;	    /* allocate TO13 to P51 */ \
        PORT->P1   &= ~(1 << 2);    /* P51 output low level */ \
        PORT->PM1  &= ~(1 << 2);    /* P51 is used as TO13 output */ \
        PORT->POM1 &= ~(1 << 2);    /* P51 is normal output mode */ \
        PORT->PMC1 &= ~(1 << 2);    /* P51 digital function */ \
}while(0)
...
#define TXD0_PORT_SETTING() do{ \
        PORT->P123CFG = 0x05;        /* allocate TXD0 to P51 */ \
        PORT->P12   |=  (1 << 3);    /* P51 output high level */ \
        PORT->PM12  &= ~(1 << 3);    /* P51 is used as TXD0 output */ \
        PORT->POM12 &= ~(1 << 3);    /* P51 is normal output mode */ \
...
#define TXD1_PORT_SETTING() do{ \
        PORT->P14CFG = 0x0A;        /* allocate TXD1 to P00 */ \
        PORT->P1   |=  (1 << 4);    /* P00 output high level */ \
        PORT->PM1  &= ~(1 << 4);    /* P00 is used as TXD1 output */ \
        PORT->POM1 &= ~(1 << 4);    /* P00 is normal output mode */ \
        PORT->PMC1 &= ~(1 << 4);    /* P00 digital function */ \
}while(0)

/* ToDo: You can allocate the RXD1 to any desired pins with RXD1PCFG register */
#define RXD1_PORT_SETTING() do{ \
        PORT->RXD1PCFG = 0x06;      /* allocate RXD1 to P01 */ \
        PORT->PM1  |=  (1 << 3);    /* P01 is used as RXD1 input */ \
        PORT->PMC1 &= ~(1 << 3);    /* P01 digital function */ \
}while(0)
...
#define TXD2_PORT_SETTING() do{ \
        PORT->P122CFG = 0x06;        /* allocate TXD2 to P13 */ \
        PORT->P12   |=  (1 << 2);    /* P13 output high level */ \
        PORT->PM12  &= ~(1 << 2);    /* P13 is used as TXD2 output */ \
        PORT->POM12 &= ~(1 << 2);    /* P13 is normal output mode */ \
        PORT->PMC12 &= ~(1 << 2);    /* P13 digital function */ \
}while(0)

/* ToDo: You can allocate the RXD2 to any desired pins with RXD2PCFG register */
#define RXD2_PORT_SETTING() do{ \
        PORT->RXD2PCFG = 0x24;      /* allocate RXD2 to P14 */ \
        PORT->PM12  |=  (1 << 1);    /* P14 is used as RXD2 input */ \
        PORT->PMC12 &= ~(1 << 1);    /* P14 digital function */ \
}while(0)
...
#define INTP0_PORT_SETTING() do{ \
        PORT->INTP0PCFG = 0x24;     /* allocate INTP0 to P136 */ \
        PORT->PM12  |=  (1 << 1);   /* P136 is used as INTP0 input */ \
        PORT->PMC12 &= ~(1 << 1);   /* P136 digital function */ \
}while(0)

/* ToDo: You can allocate the INTP1 to any desired pins with INTP1PCFG register */
#define INTP1_PORT_SETTING() do{ \
        PORT->INTP1PCFG = 0x29;     /* allocate INTP1 to P50 */ \
        PORT->PM13  |=  (1 << 6);    /* P50 is used as INTP1 input */ \
        PORT->PMC13 &= ~(1 << 6);    /* P50 digital function */ \
}while(0)
...
#define INTP3_PORT_SETTING() do{ \
        PORT->INTP3PCFG = 0x06;     /* allocate INTP3 to P30 */ \
        PORT->PM1  |=  (1 << 3);    /* P30 is used as INTP3 input */ \
        PORT->PMC1 &= ~(1 << 3);    /* P30 digital function */ \
}while(0)
