...
#include <stdio.h>
#include "base.h"
char aes_key[] = { 0xf1, 0x5b, 0xe4, 0x04, 0x40, 0x92, 0xea, 0x4e, 0xe7, 0x4e, 0xcb, 0xf7, 0x11, 0x3e, 0x47, 0xe6, 0 };
 ...
 * 打印4X4的数组
 */
void EncryptPack(const char* data, aes_int32_t len, char* result, aes_int32_t bufsz)
{
    int finishLen = 0;
    while (finishLen <= len) {
        int encryptLen = 16;
        if (len - finishLen < 16) {
            encryptLen = len - finishLen;
        }
        aes_encryption(data + finishLen, encryptLen, result + finishLen, 16);
        finishLen += 16;
    }
}
/**
...
void DecodeBlock(const char* data, char* des) //数据大小均为BLOCK_SIZE=16
{
    memcpy(des, data, BLOCK_SIZE);
    extendKey(aes_key); //扩展密钥
    aes_int32_t cArray[4][4];
    aes_int32_t i;
    copyToIntArray(des, cArray);
    ...
    copyToStrArray(cArray, des);
}

void DecodePack(const char* data, int len, char* des, int* pClearTextLen)
{
    if (len <= 0 || len % 16 != 0) {
        *pClearTextLen = 0;
        return;
    ...
    int finishLen = 0;
    while (finishLen < len) {
        DecodeBlock(data + finishLen, des + finishLen);
        finishLen += 16;
    }
    int i = 16, j = 0;
    for (i = 16; i > 0; --i) {
        for (j = 0; j < i; ++j) {
            if (des[finishLen - i + j] != i) {
                break;
            }
        }
        if (j == i) {
