...
#define TXD1_PORT_SETTING() do{ \
        PORT->P122CFG = 0x0A;        /* allocate TXD1 to P00 */ \
        PORT->P12   |=  (1 << 2);    /* P00 output high level */ \
        PORT->PM12  &= ~(1 << 2);    /* P00 is used as TXD1 output */ \
        PORT->POM12 &= ~(1 << 2);    /* P00 is normal output mode */ \
        PORT->PMC12 &= ~(1 << 2);    /* P00 digital function */ \
}while(0)
...
#define INTP1_PORT_SETTING() do{ \
        PORT->INTP1PCFG = 0x05;     /* allocate INTP1 to P50 */ \
        PORT->PM1  |=  (1 << 2);    /* P50 is used as INTP1 input */ \
        PORT->PMC1 &= ~(1 << 2);    /* P50 digital function */ \
}while(0)
