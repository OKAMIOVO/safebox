...
#define SCLA0_PORT_SETTING() do{ \
        PORT->SCLA0PCFG = 0x04;     /* allocate SCLA0 to P11 */ \
        PORT->P1   &= ~(1 << 1);    /* P11 output low level */ \
        PORT->PM1  &= ~(1 << 1);    /* P11 is used as SCLA0 output */ \
        PORT->PMC1 &= ~(1 << 1);    /* P11 digital function */ \
}while(0)

/* ToDo: You can allocate the SDAA0 to any desired pins with SDAA0PCFG register */
#define SDAA0_PORT_SETTING() do{ \
        PORT->SDAA0PCFG = 0x03;     /* allocate SDAA0 to P10 */ \
        PORT->P1   &= ~(1 << 0);    /* P10 output low level */ \
        PORT->PM1  &= ~(1 << 0);    /* P10 is used as SDAA0 inout */ \
        PORT->PMC1 &= ~(1 << 0);    /* P10 digital function */ \
}while(0)
...
#define INTP0_PORT_SETTING() do{ \
        PORT->INTP0PCFG = 0x04;     /* allocate INTP0 to P136 */ \
        PORT->PM1  |=  (1 << 1);   /* P136 is used as INTP0 input */ \
        PORT->PMC1 &= ~(1 << 1);   /* P136 digital function */ \
}while(0)
